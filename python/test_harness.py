...
MILESTONES = {
    3: ("Basic Gates Complete!", "You've built NOT, AND, OR, XOR from NAND."),
    5: ("Routing Complete!", "You can now select and distribute signals with MUX/DMUX."),
    9: ("16-bit Operations Unlocked!", "Your chips now handle 16-bit words."),
    14: ("CHAPTER 1 COMPLETE!", "You've built ALL Boolean logic chips from scratch!"),
}
...
CHIP_TESTS: list[ChipTest] = [
    ...
    ChipTest(
        ...
        truth_table=[
            # a, b, c, d, sel -> out
            (make_bus16(1), make_bus16(2), make_bus16(3), make_bus16(4), (False, False), make_bus16(1)),
            (make_bus16(1), make_bus16(2), make_bus16(3), make_bus16(4), (True, False), make_bus16(2)),
            (make_bus16(1), make_bus16(2), make_bus16(3), make_bus16(4), (False, True), make_bus16(3)),
            (make_bus16(1), make_bus16(2), make_bus16(3), make_bus16(4), (True, True), make_bus16(4)),
        ],
    ...
    ChipTest(
        ...
        truth_table=[
            # a-h, sel -> out
            (make_bus16(1), make_bus16(2), make_bus16(3), make_bus16(4),
             make_bus16(5), make_bus16(6), make_bus16(7), make_bus16(8),
             (False, False, False), make_bus16(1)),
            (make_bus16(1), make_bus16(2), make_bus16(3), make_bus16(4),
             make_bus16(5), make_bus16(6), make_bus16(7), make_bus16(8),
             (True, False, False), make_bus16(2)),
            (make_bus16(1), make_bus16(2), make_bus16(3), make_bus16(4),
             make_bus16(5), make_bus16(6), make_bus16(7), make_bus16(8),
             (False, True, False), make_bus16(3)),
            (make_bus16(1), make_bus16(2), make_bus16(3), make_bus16(4),
             make_bus16(5), make_bus16(6), make_bus16(7), make_bus16(8),
             (True, True, True), make_bus16(8)),
        ],
    ...
    for case in test.truth_table:
        *inputs, expected = case
        try:
            result = func(*inputs)
            if result is None:
                # Function returns None - not implemented yet
                return TestResult(
                    name=test.name,
                    passed=False,
...
def print_results(results: list[TestResult]):
    ...
    first_fail_idx = next((i for i, r in enumerate(results) if not r.passed), None)
    ...
    if completed > 0:
        ...
        for idx, (title, msg) in MILESTONES.items():
            if completed > idx:
                achieved_milestone = (title, msg)
        
        # Check if we just hit a milestone
        if completed - 1 in MILESTONES:
            title, msg = MILESTONES[completed - 1]
            print()
    ...
    if first_fail_idx is not None and first_fail_idx < len(CHIP_TESTS):
        ...
        print()
        print(f"  {Colors.DIM}Next: {next_chip.description} ({next_chip.name}.py){Colors.RESET}")
...
def print_single_result(result: TestResult, test: ChipTest):
    ...
    if result.passed:
        print(f"{Colors.GREEN}✓ PASSED{Colors.RESET} - All {result.total_cases} test cases passed!")
    elif result.not_implemented:
        ...
        print(f"{Colors.YELLOW}✗ FAILED{Colors.RESET} - {result.passed_cases}/{result.total_cases} cases passed")
    print()
...
Examples:
  ...
  python test_harness.py --restart     Reset all chips to stubs
    ...
    parser.add_argument(
        "chip",
        nargs="?",
        help="Specific chip to test (e.g., 'not', 'mux16')"
    )
    parser.add_argument(
        "--lint-only",
        action="store_true",
        help="Run linter only, skip tests"
    )
    parser.add_argument(
        "--restart",
        action="store_true",
        help="Reset all chips to original stub state"
    )
    parser.add_argument(
        "--no-lint",
        action="store_true",
        help="Skip linting (not recommended)"
    )
