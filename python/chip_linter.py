...
# Infrastructure files that should be skipped by linter
SKIP_FILES = {"__init__", "nand", "clock", "dff"}
...
class ChipLinter(ast.NodeVisitor):
    ...
    def visit_BoolOp(self, node: ast.BoolOp) -> None:
        ...
        if isinstance(node.op, ast.And):
            self.add_violation(
                node, "Boolean operator 'and' is forbidden - use and_gate()"
            )
        elif isinstance(node.op, ast.Or):
            self.add_violation(
                node, "Boolean operator 'or' is forbidden - use or_gate()"
            )
    ...
    def visit_UnaryOp(self, node: ast.UnaryOp) -> None:
        ...
        if isinstance(node.op, ast.Not):
            self.add_violation(
                node, "Boolean operator 'not' is forbidden - use not_gate()"
            )
    ...
    def visit_BinOp(self, node: ast.BinOp) -> None:
        ...
        if isinstance(node.op, ast.BitAnd):
            self.add_violation(
                node, "Bitwise operator '&' is forbidden - use and_gate()"
            )
        elif isinstance(node.op, ast.BitOr):
            self.add_violation(
                node, "Bitwise operator '|' is forbidden - use or_gate()"
            )
        elif isinstance(node.op, ast.BitXor):
            self.add_violation(
                node, "Bitwise operator '^' is forbidden - use xor_gate()"
            )
        self.generic_visit(node)
    ...
    def visit_Compare(self, node: ast.Compare) -> None:
        ...
        for op in node.ops:
            if isinstance(op, ast.Eq):
                self.add_violation(
                    node, "Comparison '==' is forbidden - can be used as XNOR shortcut"
                )
            elif isinstance(op, ast.NotEq):
                self.add_violation(
                    node, "Comparison '!=' is forbidden - can be used as XOR shortcut"
                )
        self.generic_visit(node)
    ...
    def visit_If(self, node: ast.If) -> None:
        ...
        if self.enforce_no_conditionals:
            self.add_violation(
                node, "Conditional 'if' is forbidden - use mux() for selection"
            )
        self.generic_visit(node)
    ...
    def visit_IfExp(self, node: ast.IfExp) -> None:
        ...
        if self.enforce_no_conditionals:
            self.add_violation(
                node, "Ternary expression is forbidden - use mux() for selection"
            )
        self.generic_visit(node)
    ...
    def visit_Import(self, node: ast.Import) -> None:
        ...
        for alias in node.names:
            self.add_violation(
                node, f"Import '{alias.name}' is forbidden - only chip imports allowed"
            )
        self.generic_visit(node)
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        ...
        if not node.module.startswith("chips"):
            self.add_violation(
                node,
                f"Import from '{node.module}' is forbidden - only 'chips' imports allowed",
            )
            return
        ...
        for alias in node.names:
            ...
            # Primitives and infrastructure are always allowed
            if imported_chip in ("nand", "clock", "dff"):
                continue
            ...
            if imported_chip not in self.allowed_imports:
                if imported_chip in CHIP_ORDER:
                    ...
                    self.add_violation(
                        node, f"Import '{imported_chip}' is not a recognized chip"
                    )
...
def lint_file(filepath: Path, enforce_no_conditionals: bool = True) -> list[Violation]:
    ...
    # Skip infrastructure and primitive files
    if chip_name in SKIP_FILES:
        return []
...
def lint_chips_directory(
    chips_dir: Path, enforce_no_conditionals: bool = True
    ...
    for chip_name in CHIP_ORDER:
        if chip_name in SKIP_FILES:
            continue  # Skip primitives and infrastructure
...
def print_lint_results(
    results: dict[str, list[Violation]], verbose: bool = True
) -> bool:
    """
