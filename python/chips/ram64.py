"""
RAM64 - 64-Register RAM
"""

from chips.ram8 import RAM8
from chips.mux8way16 import mux8way16
from chips.dmux8way import dmux8way


class RAM64:
    """
    64-register RAM - stores sixty-four 16-bit values.

    The address (6 bits) selects which register to read/write.
    If load=1, the selected register stores the input.
    Output is always the value of the selected register.

    Built from 8 RAM8 chips.

    Address breakdown:
        address[0:3] -> selects register within RAM8
        address[3:6] -> selects which RAM8
    """

    def __init__(self) -> None:
        raise NotImplementedError("ram64")

    def __call__(
        self,
        inp: tuple[bool, ...],
        load: bool,
        address: tuple[bool, bool, bool, bool, bool, bool],
    ) -> tuple[bool, ...]:
        """
        Access RAM at given address.

        Args:
            inp: 16-bit value to potentially store
            load: If True, write inp to selected address
            address: 6-bit address (selects 1 of 64 registers)

        Returns:
            16-bit value at selected address
        """
        raise NotImplementedError("ram64")
